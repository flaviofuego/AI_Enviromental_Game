...
class Table:
    ...
    def draw(self, screen):
        ...
        if self.goal_left_sprite is not None:
            # Escalar sprite al tamaño de la portería
            goal_height = int(self.goal_y2 - self.goal_y1)
            scaled_sprite = pygame.transform.scale(self.goal_left_sprite, (goal_depth * 3, goal_height))
        ...
        if self.goal_right_sprite is not None:
            # Escalar sprite al tamaño de la portería
            goal_height = int(self.goal_y2 - self.goal_y1)
            scaled_sprite = pygame.transform.scale(self.goal_right_sprite, (goal_depth * 3, goal_height))
            screen.blit(scaled_sprite, (current_width - goal_depth * 3, int(self.goal_y1)))
        else:
            # Dibujar portería por defecto
            s = pygame.Surface((goal_depth, int(self.goal_y2 - self.goal_y1)), pygame.SRCALPHA)
            s.fill((0, 255, 0, 50))
